...
def run_whisper_transcribe(audio_model, audio_path):
    ...
    Optimizations:
    - beam_size=1: greedy decoding (was 5). ~30-40% faster, minor accuracy loss
      negligible for short conversational utterances.
    - vad_filter=True: silence detection skips dead air, especially helpful when
      user holds button longer than they speak. Also reduces "Thank you" /
      "Thanks for watching" hallucinations on near-silent audio.
    - min_silence_duration_ms=500: only treat 0.5s+ as a silence boundary.
    """
    try:
